...
The canonical ledger key is ``content_sha256`` over Stage-2 canonical text.
Hawaiian maps apostrophe-like ʻokina variants to U+02BB; English folds curly
quotes/hyphen variants per ``stage2_canonical`` while preserving true ʻokina.
All text is NFC-normalized and whitespace-collapsed.
...
from llm_hawaii.stage2_canonical import (
    PAIR_DELIM as _PAIR_DELIM,
    canonical_en as normalize_en,
    canonical_haw as normalize_haw,
    canonical_pair,
    sha256_text,
)
...
def canonical_content(text_pair: Any) -> str:
    ...
    if isinstance(text_pair, Mapping):
        ...
        if en is not None and haw is not None:
            return canonical_pair(str(en), str(haw))
        if haw is not None:
    ...
    if isinstance(text_pair, Sequence) and not isinstance(text_pair, (bytes, bytearray)):
        ...
        if len(vals) == 2 and all(isinstance(v, str) for v in vals):
            return canonical_pair(vals[0], vals[1])
