#!/bin/sh
# run_stage1_eval.sh — run the Stage 1 checkpoint eval with the frozen Stage 0 bundle.
#
# This intentionally mirrors scripts/run_stage0_eval.sh so Stage 0 ↔ Stage 1
# comparisons use the same eval file, stage0.v1 prompt suite, W1 metadata probe,
# and human_fetch bidirectional translation probe.
#
# Usage:
#   CHECKPOINT=runs/llama31-8b-stage1-multisource/checkpoints/checkpoint-10100 ./scripts/run_stage1_eval.sh
#   CHECKPOINT=yashasg/llama31-8b-stage1-haw OUTPUT_DIR=data/eval_runs/stage1 ./scripts/run_stage1_eval.sh
#   RUN_NAME=checkpoint-10100 DRY_RUN=1 ./scripts/run_stage1_eval.sh
...
CHECKPOINT=${CHECKPOINT:-runs/llama31-8b-stage1-multisource/checkpoints/checkpoint-10100}
RUN_NAME=${RUN_NAME:-checkpoint-10100}
EVAL_FILE=${EVAL_FILE:-data/evals/fineweb2_haw/dev.jsonl}
OUTPUT_DIR=${OUTPUT_DIR:-data/eval_runs/stage1}
SUMMARY_DIR=${SUMMARY_DIR:-docs/eval-runs/stage1}
PROMPT=${PROMPT:-}
...
if [ ! -f "$EVAL_FILE" ]; then
    ...
    echo "Restore the local eval artifacts before running Stage 1 eval." >&2
    exit 1
fi
...
OUTPUT=${OUTPUT:-"$OUTPUT_DIR/${STAMP}__stage1_${RUN_NAME}_eval.json"}
SUMMARY=${SUMMARY:-"$SUMMARY_DIR/${STAMP}__stage1_${RUN_NAME}_eval_summary.json"}
...
"$PYTHON" - "$OUTPUT" "$SUMMARY" "$CHECKPOINT" "$EVAL_FILE" "$PROMPT" "$USE_SUITE" "$USE_MANUAL_W1" "$MANUAL_W1_JSONL" "$USE_HUMAN_FETCH" "$HUMAN_FETCH_JSONL" "$RUN_NAME" <<'PY'
import hashlib
import json
import subprocess
import sys
...
def _safe_translation_probe(probe):
    ...
    return {k: v for k, v in probe.items() if k not in ("note",)}
...
summary.write_text(json.dumps(summary_doc, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
PY

echo
echo "done. Stage 1 eval report written to:"
echo "    $OUTPUT"
echo "tracked Stage 1 summary written to:"
echo "    $SUMMARY"

if [ "${EVAL_RC:-0}" -ne 0 ]; then
    echo "warning: evaluate.py exited rc=$EVAL_RC; see artifact status." >&2
    exit "$EVAL_RC"
fi
