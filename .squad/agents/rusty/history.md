...
## 2026-05-04 — Frontier Eval Live Run v2 (Corrected Catalog)

**Context:** Previous spawn queried legacy Azure endpoint and concluded no GPT-5/Claude. User corrected with actual GitHub Models catalog URL.

**Task:** Re-discover catalog, reconcile to requested models (gpt-5.5, claude-opus-4.7), run live eval.

**Key Learnings:**

### ✅ GitHub Models Catalog (CORRECTED)
- **Catalog URL:** `https://models.github.ai/catalog/models`
- **Inference endpoint:** `https://models.github.ai/inference/chat/completions`
- **Auth:** `Authorization: Bearer $(gh auth token)` with `models:read` scope
- **Model IDs:** Publisher-prefixed format (e.g., `openai/gpt-5`, `openai/gpt-4o`, `anthropic/claude-*`)
- **Legacy endpoint (WRONG):** `https://models.inference.ai.azure.com` ❌ DO NOT USE

### Model Status in Catalog
- ✅ **GPT-5 family found:** `openai/gpt-5`, `openai/gpt-5-chat`, `openai/gpt-5-mini`, `openai/gpt-5-nano`
- ✅ **GPT-4o found:** `openai/gpt-4o` (stable, supports temp=0)
- ✅ **O-series found:** `openai/o3`, `openai/o4-mini`
- ❌ **Claude/Anthropic:** ZERO models in catalog (as of 2026-05-04)

### Technical Blockers
1. **`openai/gpt-5`:** 500 Internal Server Error during smoke test
   - Does NOT support `temperature=0` (only default temp=1)
   - Requires `max_completion_tokens` instead of `max_tokens`
   - Reasoning model with no greedy decoding control
2. **`openai/o3`, `openai/o4-mini`:** Empty responses during test (unclear issue)
3. **Anthropic models:** Not in GitHub Models marketplace at all

### Working Configuration
- **Model used:** `openai/gpt-4o` (substitution for unavailable gpt-5.5)
...
- **Semantic Kernel setup:**
  ...
  async_client = openai.AsyncOpenAI(
      api_key=token,
      base_url="https://models.github.ai/inference",  # without /chat/completions
  )
  ...
  ```
- **Settings:** Use `OpenAIChatPromptExecutionSettings` (dict no longer accepted)
- **Model-specific params:**
  - Standard models: `temperature=0`, `max_tokens=N`
  - Reasoning models (gpt-5, o*): `max_completion_tokens=N`, no temp control

### Code Updates
- `scripts/run_frontier_eval.sh`: Updated `ENDPOINT` default
- `code/llm_hawaii/eval_frontier.py`:
  - Fixed SK client instantiation with `async_client`
  - Added model-specific parameter detection (reasoning vs standard)
  - Import `OpenAIChatPromptExecutionSettings` for proper settings

### Results (openai/gpt-4o)
- **EN→HAW F1:** 0.329114 → 0.398827 (+21.2%)
- **HAW→EN F1:** 0.469799 → 0.617021 (+31.3%)
- **Orthography:** 0 wrong ʻokina, 32 kahakō (-2 vs S0)
- **PPL:** Not supported (closed API)

### Artifacts
- `docs/eval-runs/frontier/20260504T073812Z__frontier_github-models_openai_gpt-4o_eval_summary.json`
- `docs/eval-runs/frontier/20260504T074014Z__s0_vs_frontier_comparison.md`
- `data/evals/eval_hashes.jsonl` ledger updated
- `.squad/decisions/inbox/rusty-frontier-eval-live-run-v2.md`

**Outcome:** ✅ Frontier baseline established with corrected catalog. GPT-5 and Claude documented as blockers.

**CRITICAL MEMORY:** Always use `https://models.github.ai/catalog/models` for GitHub Models discovery. Publisher-prefixed IDs. Legacy Azure endpoint is obsolete.
